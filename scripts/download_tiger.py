#!/usr/bin/env python3
"""Download public TIGER files from the open S3 bucket.

Examples:
    python scripts/download_tiger.py --prefix wsirois/ --max_files 25
    python scripts/download_tiger.py --prefix wsibulk/ --output_dir data/tiger
"""
...
def download_tiger_dataset(output_dir: str, prefix: str, max_files: int | None) -> tuple[int, int, int]:
    ...
    if not files_to_download:
        ...
        print("    Try one of: wsirois/, wsibulk/, wsitils/")
        return 0, 0, 0
    ...
    return len(files_to_download), downloaded, skipped
...
def main():
    ...
    if args.max_files is not None and args.max_files < 0:
        parser.error("--max_files must be >= 0")

    prefix = _normalize_prefix(args.prefix)

    os.makedirs(args.output_dir, exist_ok=True)
    total, downloaded, skipped = download_tiger_dataset(args.output_dir, prefix, args.max_files)
    print(f"\n[+] Download complete. listed={total}, downloaded={downloaded}, skipped={skipped}")
